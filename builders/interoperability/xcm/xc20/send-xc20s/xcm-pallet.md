...
categories:
- XC-20
url: https://docs.moonbeam.network/builders/interoperability/xcm/xc20/send-xc20s/xcm-pallet/
word_count: 2271
token_estimate: 3924
version_hash: sha256:07268eacfc510c766e799b5af7c460c91cfc1a77e099f2b756d2d228265d4f83
...
Manually crafting an XCM message for fungible asset transfers is a challenging task. Consequently, developers can leverage wrapper functions and pallets to use XCM features on Polkadot and Kusama. One example of such wrappers is the [XCM](https://docs.polkadot.com/chain-interactions/send-transactions/interoperability/transfer-assets-parachains/) Pallet, which provides different methods to transfer fungible assets via XCM.

This guide will show you how to leverage the Polkadot XCM Pallet to send [XC-20s](/moonbeam-mkdocs/builders/interoperability/xcm/xc20/overview/) from a Moonbeam-based network to other chains in the ecosystem (relay chain/parachains).
...
The Polkadot XCM Pallet includes the following read-only storage methods. Note, this is not an exhaustive list. To see the current available storage methods, check the [Chain State of Polkadot.js Apps](https://polkadot.js.org/apps/#/chainstate).
...
To check your xcUNIT balance, you can add the XC-20's [precompile address](/moonbeam-mkdocs/builders/interoperability/xcm/xc20/overview/#calculate-xc20-address) to MetaMask with the following address:
...
In this example, you'll build an XCM message to transfer xcUNIT from Moonbase Alpha back to the Alphanet relay chain through the `transferAssets` function of the Polkadot XCM Pallet using the [Polkadot.js API](/moonbeam-mkdocs/builders/substrate/libraries/polkadot-js-api/).
...
If you're having difficulty replicating the demo, take the following troubleshooting steps:
 ...
 - Check the [Explorer](https://polkadot.js.org/apps/?rpc=wss%3A%2F%2Fwss.api.moonbase.moonbeam.network#/explorer) on Polkadot.js Apps on Moonbase Alpha to ensure a successful transaction on the origin chain
 - Check the [Explorer](https://polkadot.js.org/apps/?rpc=wss%3A%2F%2Frelay.api.moonbase.moonbeam.network#/explorer) on Polkadot.js Apps and review the XCM messages received on Moonbase Relay Chain
